...
function main() {
  ...
  // ── Détection du motif « fonction » ──────────────────────────────
  // Le nom de la fonction ne doit pas décider du mode d'affichage : f, g, h ou
  // toute autre lettre unique valent la courbe. Le motif reste ancré pour ne
  // pas avaler un libellé quelconque contenant des parenthèses.
  console.log('\n── Détection du motif « fonction » ──')
  const ligne = (libelle) => ({ headers: ['x', '1', '2', '3'], rows: [[libelle, '2', '4', '6']] })
  for (const lettre of ['f', 'g', 'h', 'u', 'p']) {
    check(`« ${lettre}(x) » détecté comme fonction`, detecterTypeGraphique(ligne(`${lettre}(x)`)) === 'fonction')
  }
  check('« G(x) » : la casse ne change rien', detecterTypeGraphique(ligne('G(x)')) === 'fonction')
  check('« g (x) » : les espaces ne changent rien', detecterTypeGraphique(ligne('g (x)')) === 'fonction')
  check('« Effectif » n’est pas une fonction', detecterTypeGraphique(ligne('Effectif')) !== 'fonction')
  check('« Prix (€) » n’est pas une fonction', detecterTypeGraphique(ligne('Prix (€)')) !== 'fonction')
  check('« Aire (x) » n’est pas une fonction', detecterTypeGraphique(ligne('Aire (x)')) !== 'fonction')
  check('« fg(x) » (deux lettres) n’est pas une fonction', detecterTypeGraphique(ligne('fg(x)')) !== 'fonction')
  check('« f(y) » n’est pas une fonction', detecterTypeGraphique(ligne('f(y)')) !== 'fonction')
  // Resserrement assumé par rapport à l'ancien `includes('f(x)')` : le libellé
  // doit être le motif entier, pas le contenir.
  check('« f(x) en cm » n’est pas une fonction', detecterTypeGraphique(ligne('f(x) en cm')) !== 'fonction')
  check('en-tête autre que « x » : pas de fonction',
    detecterTypeGraphique({ headers: ['Année', '1', '2'], rows: [['g(x)', '2', '4']] }) === null)
  // Le mot de substitution suit le type détecté : « graphique » pour une fonction.
  modeAffichage[0] = 'graphique'
  checkRendu('g(x) : l’énoncé bascule sur « graphique »',
    'Le tableau ci-dessous représente la fonction g.',
    'Le graphique ci-dessous représente la fonction g.', ligne('g(x)'))
  delete modeAffichage[0]

  // ── Câblage ──────────────────────────────────────────────────────
  // Les cas ci-dessous exercent `adapterEnonce` en isolation : ils resteraient
  // tous verts si un remaniement de renderQuiz rebranchait l'affichage sur
  // `q.q` et débranchait la fonction. On contrôle donc le câblage dans le
  // source lui-même, aux trois points où l'énoncé est rendu : les deux
  // branches de renderQuiz (quiz en cours, quiz terminé) et renderResultats.
  console.log('\n── Câblage aux trois points de rendu ──')
  const compter = (aiguille) => src.split(aiguille).length - 1
  // La déclaration `function adapterEnonce(q,i){` contient elle aussi le
  // motif : elle est retirée du compte pour ne laisser que les appels.
  const nbAppels = compter('adapterEnonce(q,i)') - compter('function adapterEnonce(q,i)')
  check('quiz.html ne rend plus l’énoncé brut (<p class="q-text">${q.q}</p>)',
    compter('<p class="q-text">${q.q}</p>') === 0)
  check('quiz.html appelle adapterEnonce(q,i) exactement 3 fois',
    nbAppels === 3, `${nbAppels} appel(s)`)
  check('les 3 appels sont bien aux points de rendu de l’énoncé',
    compter('<p class="q-text">${adapterEnonce(q,i)}</p>') === 3)
