...
// package.json ne déclare pas "type": "module", donc node classe tout .js
// comme CommonJS — or api/generer.js et api/examen.js utilisent
// `export default`. Depuis node 22.7, la détection de syntaxe rattrape le
// coup : le fichier est reparsé en ESM, au prix d'un avertissement
// MODULE_TYPELESS_PACKAGE_JSON. Sur node 20 le chargement échoue.
// On recopie donc les sources sous l'extension .mjs, qui lève l'ambiguïté
// quelle que soit la version de node, en conservant l'arborescence
// api/ + lib/ pour que l'import relatif '../lib/questions-vues.js' résolve.
// Le test porte ainsi toujours sur le code réel du dépôt, jamais sur une
// copie figée.
//
// Cette copie n'affaiblit pas ce qui est vérifié : le répertoire temporaire
// ne contient aucun package.json, donc lib/questions-vues.js y est classé
// CommonJS exactement comme dans le dépôt. Le harnais exerce donc bien un
// import ESM NOMMÉ depuis un module CommonJS — le mécanisme même du
// refactor, et le seul qui pourrait casser les deux endpoints d'un coup.
//
...
function preparerCopies() {
  ...
  // examen-score.js et auth-eleve.js : extraits de api/examen.js par la PR #71
  // (écriture serveur), jamais ajoutés ici depuis — sans eux l'import de
  // api/examen.mjs échoue avant le premier cas (ERR_MODULE_NOT_FOUND),
  // découvert en repassant ce test pour le chantier reprise de session
  // (21/09/2026). Bug pré-existant, sans rapport avec ce chantier.
  fs.copyFileSync(path.join(RACINE, 'lib/examen-score.js'), path.join(tmp, 'lib/examen-score.js'))
  fs.copyFileSync(path.join(RACINE, 'lib/auth-eleve.js'), path.join(tmp, 'lib/auth-eleve.js'))
...
// 21/09/2026 après le 500 constaté sur F5 — colonne "tableau" demandée par
// 'reprise' mais inexistante). Utilisé par les mocks de fetch ci-dessous
// pour rejeter, comme le ferait vraiment PostgREST (42703), toute colonne
// qui n'existe pas — un mock qui accepte n'importe quel select= ne peut pas
// détecter ce genre de régression.
const COLONNES_EXAMEN_QUESTIONS_REELLES = new Set(['id', 'numero', 'theme', 'question', 'opts', 'answer', 'explication', 'annee', 'partie', 'chapitre', 'figure', 'figure_url'])

function verifierSelectExamenQuestions(url) {
  if (!url.includes('examen_questions')) return null
  const m = url.match(/select=([^&]*)/)
  if (!m || m[1] === '*') return null
  const demandees = decodeURIComponent(m[1]).split(',')
  const inconnue = demandees.find(c => !COLONNES_EXAMEN_QUESTIONS_REELLES.has(c))
  if (!inconnue) return null
  const corps = { code: '42703', message: `column examen_questions.${inconnue} does not exist` }
  return { ok: false, status: 400, text: async () => JSON.stringify(corps), json: async () => corps }
}
...
async function main() {
  ...
  console.log('\n╔══ EXAMEN BLANC (api/examen.js, source=examen) ══╗')
  {
    ...
    function installFetch(db) {
      global.fetch = async (url, opts = {}) => {
        const method = opts.method || 'GET'
        // Authentification (cf. chantier reprise de session, 21/09/2026) :
        // api/examen.js vérifie désormais le jeton via /auth/v1/user avant
        // toute action, exactement comme api/stripe-checkout.js. Ce mock ne
        // compte pas dans db.appels (distinct des appels /rest/v1/ déjà
        // vérifiés par les tests existants) : seul un jeton correspondant à
        // JETON résout vers SUB, tout le reste (dont l'absence de jeton,
        // court-circuitée avant même l'appel réseau par verifierToken)
        // échoue en 401.
        if (url.includes('/auth/v1/user')) {
          const authHeader = (opts.headers && opts.headers['Authorization']) || ''
          if (authHeader === `Bearer ${JETON}`) return { ok: true, json: async () => ({ id: SUB, email: 'eleve@test.fr' }) }
          return { ok: false }
        }
        // Vérifie le select= contre les colonnes réelles de examen_questions
        // (information_schema, 21/09/2026) — le mock précédent acceptait
        // n'importe quelle colonne, ce qui a laissé passer une demande sur
        // "tableau" (colonne inexistante) jusqu'en preview, en 500 muet côté
        // Vercel avant l'ajout des console.error de ce chantier.
        const erreurColonne = verifierSelectExamenQuestions(url)
        if (erreurColonne) return erreurColonne
        db.appels.push(`${method} ${url.split('/rest/v1/')[1] || url}`)
        if (url.includes('questions_vues')) {
          const op = method === 'GET' ? 'lecture' : method === 'DELETE' ? 'purge' : 'marquage'
          if (db.echec && db.echec(op)) {
    ...
    console.log('\n── 4. Partie sous le seuil : comportement identique à l\'existant ──')
    {
      ...
      // Un jeton absent renvoie désormais 401 (cf. section 6) : la référence
      // "non filtré" utilise un jeton valide sur des vues vierges — premier
      // passage, rien de déjà vu, donc aucune question écartée par le
      // filtrage. Équivalent en nombre de questions à l'ancien comportement
      // anonyme, seule la métrique comparée ici (r.questions.length).
      const ref = await run(dbRef, JETON)
    ...
    console.log('\n── 6. Jeton absent : authentification refusée, clé anon absente ──')
    {
      // Avant ce chantier, un jeton absent faisait tourner l'examen en mode
      // anonyme (raison=anonyme, non filtré). Ce n'est plus vrai : les 4
      // actions de api/examen.js vérifient désormais le jeton avant tout —
      // examens_progression n'a aucune policy RLS, sa seule protection est
      // cette vérification (cf. db/policies.sql). Sans jeton valide, 401,
      // aucun appel réseau (verifierToken court-circuite avant fetch).
      const db = makeDB()
      const r = await run(db, null)
      check('sans jeton : 401, aucun examen servi', r.code === 401 && !r.questions, JSON.stringify(r))
      check('sans jeton : aucun appel réseau émis', db.appels.length === 0, db.appels.join(', '))

      // Jeton valide mais clé anon absente : failure mode DIFFÉRENT et
      // toujours fail-open — ne concerne que le filtrage questions_vues, pas
      // l'authentification (qui repose sur SUPABASE_SERVICE_KEY, toujours
      // présente ici).
