// /api/cron-rappel-test.js
//
// ⚠️ FICHIER DE TEST TEMPORAIRE — NE PAS DÉPLOYER EN PROD TEL QUEL ⚠️
// Différence unique avec cron-rappel.js : les branches "été" (juillet) et
// "août" sont court-circuitées (if (false) au lieu de if (mois === 7)/(mois === 8))
// pour pouvoir atteindre et tester la branche "bilan périodique" dès maintenant.
// Une fois le test validé, supprimer ce fichier — ne jamais le laisser en prod,
// sinon plus aucun email été/août ne partira jamais.
//
// ═══════════════════════════════════════════════════════════
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⚠️ TEST : désactivé temporairement (if (false)) pour atteindre la branche bilan
    if (false && mois === 7) {
      let envoyes = 0
      for (const profil of profils) {
        const { user_id, prenom_affiche, email_parent, email_actif } = profil
