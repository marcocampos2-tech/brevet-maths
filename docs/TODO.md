...
29. ✅ **Fermé (23/09/2026, lot « session corrompue ») — boucle `connexion.html`** : confirmé — `connexion.html` (`init()`) traitait tout `data.session` non nul comme une session valide sans la vérifier côté serveur, puis son propre fail-open (`errProfil` → redirection immédiate vers `quiz.html`) renvoyait l'élève vers l'appli SANS JAMAIS afficher le formulaire — la session corrompue n'était donc jamais remplacée par une vraie connexion. Corrigé : `init()` valide désormais le jeton via `sb.auth.getUser()` (aller-retour serveur), avec une tentative de `refreshSession()` en cas de refus d'authentification (status 4xx sauf 429, hors `AuthRetryableFetchError`) — purge locale (`purgerSessionLocale()`, `signOut({scope:'local'})` + repli manuel sur `localStorage`, même méthode que `examen.html`) uniquement si cette classification confirme un jeton mort, jamais sur une simple panne technique (réseau, 5xx, 429, absence de status). Le fail-open de la lecture `profils` après une session validée ne redirige plus à l'aveugle vers `quiz.html` : à ce stade, l'ancien risque qu'il couvrait (jeton mort) n'existe plus, alors que le risque qu'il faisait courir (un compte parent redirigé vers le quiz de son enfant) restait entier.

    **Reste ouvert — pages dégradées** : même motif de gate (session "présente" acceptée sans vérification serveur) non audité individuellement sur `quiz.html` (sa propre garde `init()`, distincte du mécanisme de sauvegarde traité au lot précédent), `prof.html`, `suivi-parent.html`, `resultats.html`, `espace-parent.html`, `abonnement-confirme.html` — à vérifier au cas par cas avant de généraliser ce correctif.

    **`purgerSessionLocale()` existe désormais en 3 copies** (`examen.html`, `connexion.html`, `quiz.html`) — même code dupliqué à chaque fois plutôt que factorisé (choix délibéré de ce lot, cf. discussion de conception). **`estRefusAuthentification()` existe désormais en 2 copies** (`connexion.html`, `quiz.html`) — même choix, même raison (le refus d'authentification confirmé sur `refreshSession()` doit être distingué d'une panne technique aussi bien dans la garde de connexion que dans `tenterSauvegarde()`). Les deux, candidates à un futur helper client partagé — aucun module JS commun côté client n'existe aujourd'hui dans ce dépôt, contrairement à `lib/` côté serveur.
