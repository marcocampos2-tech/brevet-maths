...
- [ ] **Deux seuils différents et volontairement désalignés dans `suivi-parent.html`** (carte « Niveau atteint par sous-thème ») : un sous-thème est compté « abordé » (ligne 1 de la synthèse) dès 1 session, mais reste classé « À découvrir » tant qu'il a moins de 3 sessions (`calculerEtatSousTheme`). Un sous-thème à 1 session apparaît donc dans les deux à la fois — assumé, pas un bug. Ne pas aligner les deux seuils sans en faire un chantier séparé (changement de comportement plus large que la simple formulation de la phrase).
