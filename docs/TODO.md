...
20. **Corrigé (20/09/2026)** — `renderResultats()` (`examen.html`) conditionne désormais le message de succès sur `resultatServeur !== null` : si `sauvegarder()` a échoué (`data.success` faux ou exception réseau), un message honnête d'erreur remplace « ✅ Résultats enregistrés ». Le score 0/20 affiché dans ce cas reste un signal honnête d'un problème, pas un vrai score à corriger davantage : sans `resultatServeur`, il n'y a de toute façon aucune correction disponible à afficher à la place. Correctif minimal volontaire — pas de retry, pas de nouveau mécanisme.

    **Retry automatique envisagé puis écarté (20/09/2026)** : aucun trafic actuel, aucun signalement, aucune donnée sur les causes possibles d'échec. Un retry ne protégerait que contre une indisponibilité transitoire du serveur ; si la cause est autre (donnée malformée, bug de correction, timeout structurel), il échouerait à l'identique en ajoutant 2-3 secondes d'écran figé pour rien. Le message honnête, lui, aide dans tous les cas. À rouvrir si un incident réel est un jour observé.
...
29. **`connexion.html` : une session "présente" mais corrompue (jeton invalide) fait boucler l'élève sans jamais atteindre le formulaire de connexion** — trouvé le 22/09/2026 en corrigeant le bouton "Se reconnecter" de `examen.html` (chantier reprise de session). `connexion.html` (`init()`, ~l.115-134) traite tout `data.session` non nul comme une session valide sans la vérifier, puis son propre fail-open (`errProfil` → redirection immédiate vers `quiz.html`, pas juste un `console.log`) renvoie l'élève vers l'appli SANS JAMAIS afficher le formulaire — la session corrompue n'est donc jamais remplacée par une vraie connexion, et le cycle recommence à l'identique. Corrigé côté `examen.html` (le bouton "Se reconnecter" appelle désormais `sb.auth.signOut({scope:'local'})`, avec repli sur un nettoyage manuel du `localStorage`, avant de rediriger). `connexion.html` lui-même non corrigé ici — son gate reste vulnérable au même scénario si l'élève y arrive par un autre chemin (favori, lien direct) sans passer par le bouton corrigé. Même motif de gate (session "présente" acceptée sans vérification + fail-open qui redirige plutôt que de simplement journaliser) potentiellement présent sur `quiz.html`, `prof.html`, `suivi-parent.html`, `resultats.html`, `espace-parent.html`, `abonnement-confirme.html` — pas audités individuellement, à vérifier au cas par cas avant de généraliser un correctif.
