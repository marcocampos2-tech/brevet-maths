...
- [ ] **REFONTE `suivi-parent.html` (indicateur "niveau atteint") livrée, en attente de merge** — [PR #43](https://github.com/marcocampos2-tech/brevet-maths/pull/43), 07/09/2026. Diverge du plan initial sur deux points tranchés en conversation avant exécution : dénominateur par la liste `SOUS_THEMES` codée en dur (recopiée depuis `quiz.html`), pas de `COUNT` dynamique sur `questions_banque` (lecture publique retirée par l'audit sécurité — pas de nouvelle vue Postgres créée pour ce chantier) ; pas de module partagé `lib/progression.js` — cf. risque actif dédié en section technique ci-dessous. États renommés `difficulte`/`acquis` (au lieu de `en_difficulte`/`maitrise`), seuils inchangés (5 sessions ≥70% par difficulté, 6 tentatives sans déblocage). `cron-rappel.js` non touché : la comparaison "depuis le dernier bilan" reste en mode dégradé (phrase descriptive systématique) jusqu'à la PR d'alignement — cf. point 17 ci-dessous, toujours ouvert.
...
19. **À vérifier le 06/09/2026 au matin** : `select * from historique_bilans` doit renvoyer 2 lignes (bilans de Marco et Test, cadence de 21 jours atteinte le 06/09) et deux emails doivent être arrivés. Si 0 ligne : soit le cron ne se déclenche pas automatiquement (incident du 20/08 non résolu, cf. point 11), soit le `POST` échoue silencieusement (`emailRes.ok` conditionne l'écriture dans `api/cron-rappel.js` mais rien n'inspecte la réponse du `POST` lui-même, l.449-450) — à distinguer en regardant les logs Vercel **le matin même**, la rétention n'étant que d'une heure sur le plan gratuit
20. **⚠️ RISQUE ACTIF — duplication `SOUS_THEMES` + algorithme de déblocage entre `quiz.html` et `suivi-parent.html` (depuis PR #43, pas de `lib/progression.js` partagé).** Pas une note historique : **à contrôler à chaque future modification touchant l'un des deux fichiers** (ajout/retrait d'un sous-thème, changement de `SEUIL_SESSIONS`/`SEUIL_PCT`, modification de la logique de `verifierDeblocage()`). Une modification sur un seul des deux fichiers fait diverger silencieusement ce que l'élève vit dans `quiz.html` et ce que le parent voit dans `suivi-parent.html` — aucune erreur, aucun log, juste un état affiché faux. Avant de merger un changement sur l'un des deux fichiers touchant à ces éléments, vérifier explicitement si l'autre fichier doit être répercuté. Ne cocher/retirer ce point que si un module partagé est mis en place (auquel cas vérifier d'abord que Vercel sert bien `/lib/*.js`, cf. l'ancien plan de refonte).
