...
20. **Corrigé (15/09/2026)** — pas de retour vers l'accueil depuis `suivi-parent.html` (logo non cliquable, aucune flèche retour, contrairement à `espace-parent.html` qui en a une ; constaté le 15/09 en test mobile) : lien "← Retour à l'accueil" ajouté dans `nav-right`, identique à `espace-parent.html`
21. **Corrigé (15/09/2026)** — pas d'œil pour révéler le mot de passe sur `suivi-parent.html` (formulaire de création d'enfant) : œil ajouté sur `#fa-password` (`toggleFaMdp()`), composant répliqué de `connexion.html`. Vérification faite sur la saisie élève de `connexion.html` (`#password`) : elle a déjà son œil (`toggleMdp()`) depuis l'origine — aucune action nécessaire sur ce fichier
22. **Clos (15/09/2026)** — découvrabilité du reset de mot de passe : vérification faite, le problème était surestimé. Le lien "Mot de passe oublié ?" est visible dès l'arrivée sur connexion.html, juste sous le champ mot de passe, sans action préalable — c'est-à-dire exactement là où un élève qui échoue à se connecter, ou un parent qui l'aide, va le chercher naturellement. La gate A+B+C ne bloque que le cas particulier d'un parent DÉJÀ connecté à son espace qui tape l'URL connexion.html ; ce n'est pas le chemin naturel, puisque l'échec de connexion se produit côté élève, sur une session non-parent. Le mécanisme lui-même est validé de bout en bout depuis le 14/09 (multi-enfants, rate-limit par enfant, écran de recovery). Aucune action nécessaire. Si un parent réel signale un jour être bloqué, rouvrir — mais ne pas coder préventivement.
